...
func RedisMarshalWithGob(data interface{}) *bytes.Buffer {
	...
	if err != nil {
		debugger.LogError("RedisMarshalWithGob:", err)
		return nil
	}
	return buf
}
...
func RedisUnmarshalWithGob(data []byte, reply interface{}) {
	...
	if err != nil {
		debugger.LogError("RedisUnmarshalWithGob:", err)
	}
}
...
func RedisMarshal(data interface{}) []byte {
	...
	if err != nil {
		debugger.LogError("RedisMarshal:", err)
		return nil
	}
	// ccgo.LogDebug("dbprox/redis.go - buf info: ", string(buf))
	return buf
}
...
func RedisUnmarshal(data []byte, reply interface{}) {
	...
	if err != nil {
		debugger.LogError("RedisUnmarshal:", err)
	}
}
...
func newRedisProxy(conf RedisConfig) *Redis {
	...
	debugger.LogInfo("newRedisProxy connected to", conf)
	return r
}
