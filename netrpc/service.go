...
func (owner *Service) Start() {
	...
	if err != nil {
		debugger.LogError("netrpc service.go - Start:", err)
		return
	}
	owner.listener = l
	...
	if regErr != nil {
		debugger.LogError("netrpc service.go - Register proxy:", regErr)
		return
	}
	...
	for {
		...
		if err != nil {
			debugger.LogError("netrpc service.go - Start Accept Error:", err)
			break
		}
		owner.conn = conn
