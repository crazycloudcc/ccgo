...
func (owner *Service) Start() {
	owner.doListen()
	go owner.doAccept()
	debugger.LogInfo("nettcp.Service Started with:", owner.conf.String())
}
...
func (owner *Service) doListen() {
	...
	if err != nil {
		debugger.LogFatal("doListen net.Listen:", err)
		return
	}
...
func (owner *Service) doAccept() {
	for {
		...
		if err != nil {
			debugger.LogWarn("doAccept owner.listener.Accept:", err)
			continue
		}

		owner.clientMngr.addConnection(newClient(conn, owner))
		debugger.LogDebug("New Client From:", conn.RemoteAddr().String())
	}
}
