...
type Client struct {
	...
	chanMsg       *ccgo.Chan    // message channel.
...
func (owner *Client) Close() {
	...
	ccgo.LogDebug("client.go - Close: ", owner.id)
...
func (owner *Client) Send(msg interface{}) {
	m := msg.(*Msg)
	ccgo.LogDebug(fmt.Sprintf("client.go - send msglen:[%v], msgid:[%v]", m.Len, m.ID))

	buf, err := Marshal(m)
	if err != nil {
		ccgo.LogError("client.go - send error: ", owner.id, err)
		return
	}
	owner.doWrite(buf)
}
...
func newClient(c net.Conn, serv *Service) *Client {
	...
	tc.chanMsg = ccgo.NewChan(MsgChanMax, tc.doParse)
	tc.readLen = 0
	tc.writeLen = 0
	tc.serv = serv
	return tc
...
func (owner *Client) doClose() {
	...
	owner.conn.Close()
	owner.chanMsg.Close()
	ccgo.LogDebug("client.go - doClose: ", owner.id)
}
...
func (owner *Client) doRead() {
	for {
		...
		if err != nil {
			if err.Error() != "EOF" {
				ccgo.LogWarn("client.go - read error: ", owner.id, err)
			}
			break
		}
		owner.readLen += int32(len)
		owner.chanMsg.Write(buf[:len])
		ccgo.LogDebug(fmt.Sprintf("client.go - id: [%d] read len: [%d], total read len: [%d]", owner.id, len, owner.readLen))
	}
	owner.Close()
}
...
func (owner *Client) doWrite(buf []byte) {
	...
	if err != nil {
		ccgo.LogError("client.go - write error: ", owner.id, err)
		owner.Close()
		return
	}
	owner.writeLen += int32(len)
	ccgo.LogDebug(fmt.Sprintf("client.go - id: [%d] write len: [%d], total write len: [%d]", owner.id, len, owner.writeLen))
}
...
func (owner *Client) doParse(b interface{}) {
	...
	for {
		if owner.buffer.Len() < MsgHeadLen {
			// ccgo.LogWarn(fmt.Sprintf("client.go - owner.buffer.Len()[%d] < MsgHeadLen[%d]", owner.buffer.Len(), MsgHeadLen))
			break
		}
		msgLen := owner.buffer.Bytes()[:MsgLenIndexE]
		l, err := ReadMsgLen(msgLen)
		if err != nil {
			ccgo.LogError("client.go - ReadMsgLen error: ", owner.id, err)
			owner.Close()
			break
		}
		if int(l)+MsgHeadLen > owner.buffer.Len() {
			// ccgo.LogWarn(fmt.Sprintf("client.go - int(l)+MsgHeadLen [%d] > owner.buffer.Len() [%d]", int(l)+MsgHeadLen, owner.buffer.Len()))
			break
		}
		...
		if err != nil {
			ccgo.LogError("client.go - Unmarshal error: ", owner.id, err)
			owner.Close()
			break
		}
