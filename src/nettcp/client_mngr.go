...
type ClientMngr struct {
	...
	hashClients *datastructs.Hash // connection id -> *Client.
	hashUIDs    *datastructs.Hash // connection id -> uid.
}
...
func newClientMngr(max int32) *ClientMngr {
	tcm := new(ClientMngr)
	tcm.hashClients = datastructs.NewHash(1)
	tcm.hashUIDs = datastructs.NewHash(1)
	return tcm
}
