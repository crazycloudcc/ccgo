...
func (owner *Router) RegHandler(msgid uint32, handler interface{}) {
	ccgo.LogDebug("router.go - register hander with msgid: ", msgid)
...
func newRouter(max int32) *Router {
	...
	r.chanMsg = ccgo.NewChan(max, r.doRoute)
...
func (owner *Router) doRoute(data interface{}) {
	ev := data.(*Event)
	ccgo.LogDebug(fmt.Sprintf("route.go - connid:[%d] msgid:[%d] len:[%d]", ev.Connid, ev.Msgid, len(ev.MetaData)))
	...
	if handler != nil {
		...
		// d := time.Now().Sub(s)
		// ccgo.LogDebug(fmt.Sprintf("router.go - doRoute connid:[%d] msgid:[%d] cost time:[%v]", ev.Connid, ev.Msgid, d))
		////////////////////////////////////////////////////////
		return
	}
	ccgo.LogWarn("router.go - not found msg handler: ", ev.Connid, ev.Msgid)
}
