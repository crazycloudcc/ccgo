...
func (owner *Service) Start() {
	owner.doListen()
	go owner.doAccept()
	ccgo.LogInfo("nettcp.Service Started with:", owner.conf.String())
}
...
func (owner *Service) doListen() {
	...
	if err != nil {
		ccgo.LogFatal("doListen net.Listen:", err)
		return
	}
...
func (owner *Service) doAccept() {
	for {
		...
		if err != nil {
			ccgo.LogWarn("doAccept owner.listener.Accept:", err)
			continue
		}

		owner.clientMngr.addConnection(newClient(conn, owner))
		ccgo.LogDebug("New Client From:", conn.RemoteAddr().String())
	}
}
