...
func (owner *Client) close() {
	if owner.id == -1 {
		ccgo.LogDebug("client.go closed -------", owner.id, owner.chanRes)
		return
	}
	ccgo.LogDebug("client.go close -------", owner.id, owner.chanRes)
	owner.conn.Close()
	close(owner.chanRes)
	owner.id = -1
}
...
func (owner *Client) read() {
	defer func() {
		ccgo.LogDebug("client read unregister: ", owner.id)
	...
	for {
		...
		if err != nil {
			ccgo.LogDebug("client read error: ", owner.id, err)
			break
		}
		ccgo.LogDebug("client read data: ", owner.id, len(buf))
		msg := bytesToMsg(buf)
...
func (owner *Client) write() {
	defer func() {
		ccgo.LogDebug("client write error: ", owner.id)
		owner.serv.GetClientMngr().unregister <- owner.id
	}()
	...
	for {
		...
		case buf := <-owner.chanRes:
			...
			// ccgo.LogDebug("client write data: ", owner.id, buf)
